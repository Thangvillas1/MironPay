...
export default function LaunchpadSubmitPage() {
  ...
  const requiredOk = name && sym && mark && category && tagline && blurb && supply && price && target && minRaise && cap && minContribution && startAt && endAt && tokenAddress
    && parseFloat(minRaise || '0') <= parseFloat(target || '0')
  ...
  async function submitWithPin(enteredPin: string) {
    ...
    try {
      const res = await fetch('/api/launchpad/submit', {
        ...
        body: JSON.stringify({
          pin: enteredPin, name, sym: sym.toUpperCase(), mark: mark.toUpperCase().slice(0, 2), accent, category, tagline, blurb, minRaise, tokenAddress,
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '28px 34px 80px' }}>
        ...
        <Section title="Project info">
          ...
          <Field
            ...
            hint="Your own already-deployed ERC-20. MironPay does not create tokens — you deposit this token's supply into the sale contract yourself after approval."
          >
            <input style={{ ...inputStyle, fontFamily: 'monospace' }} value={tokenAddress} onChange={e => setTokenAddress(e.target.value.trim())} placeholder="0x..." />
          </Field>
        </Section>
        ...
        <Section title="Sale details">
          ...
          <Field
            label="Softcap / minimum raise (USD) *"
            hint="If this isn't reached by the sale end, contributors are refunded automatically."
          >
            <input style={inputStyle} value={minRaise} onChange={e => setMinRaise(e.target.value.replace(/[^0-9.]/g, ''))} placeholder="300000" />
          </Field>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        ...
        <div style={{ paddingTop: 22, marginTop: 22, borderTop: '1px solid rgba(var(--c-fg-rgb),.07)' }}>
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontSize: 12.5, fontWeight: 700, color: 'var(--c-muted2)' }}
          >
            <span style={{ display: 'inline-block', transition: 'transform .15s', transform: showMore ? 'rotate(90deg)' : 'none' }}>›</span>
            {showMore ? 'Hide' : 'Add'} more details (optional — team, socials, tokenomics breakdown)
          </button>
          ...
          {showMore && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginTop: 16 }}>
              ...
              <Field label="Token distribution (one per line: Label, Percent)" hint="Percentages should add up to 100.">
