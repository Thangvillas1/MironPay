...
import LaunchpadContributeModal from '@/app/components/LaunchpadContributeModal'
...
interface ProjectDetail {
  ...
  minRaise: number; softcapMet: boolean
  myContribution: number
  onChainRegistered: boolean
  isOwnerOrAdmin: boolean
  saleIdHash: string | null
...
export default function LaunchpadProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [refundStep, setRefundStep] = useState<'idle' | 'pin' | 'progress' | 'done' | 'error'>('idle')
  const [refundError, setRefundError] = useState<string | null>(null)
  const [refundTxHash, setRefundTxHash] = useState<string | null>(null)
  const [claimStep, setClaimStep] = useState<'idle' | 'pin' | 'progress' | 'done' | 'error'>('idle')
  ...
  async function handleRefund(pin: string) {
    setRefundStep('progress')
    const { data: { session } } = await supabase.auth.getSession()
    const res = await fetch(`/api/launchpad/sales/${id}/refund`, {
      method: 'POST',
    ...
    setRefundTxHash(data.txHash ?? null)
    setRefundStep('done')
  }

  async function handleClaim(pin: string) {
    setClaimStep('progress')
    const { data: { session } } = await supabase.auth.getSession()
    const res = await fetch(`/api/launchpad/sales/${id}/claim`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${session?.access_token ?? ''}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ pin }),
    })
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--c-page)', color: 'var(--c-text)' }}>
          ...
          <aside style={{ position: 'sticky', top: 24, display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ padding: 20, borderRadius: 18, background: 'var(--glass-bg)', backdropFilter: 'blur(var(--glass-blur))', WebkitBackdropFilter: 'blur(var(--glass-blur))', border: '1px solid var(--glass-border)', boxShadow: 'var(--glow-primary), inset 0 1px 0 var(--glass-hi)' }}>
              ...
              {isLive && !showAgentPrompt && (
                <>
                  <button className="lp-join" onClick={() => setShowContributeModal(true)} disabled={!project.onChainRegistered} style={{ width: '100%', height: 52, marginTop: 16, borderRadius: 14, border: 'none', fontSize: 15, fontWeight: 600, cursor: project.onChainRegistered ? 'pointer' : 'default', transition: 'transform .15s,filter .15s', background: 'var(--grad-primary)', boxShadow: 'var(--glow-primary)', color: '#fff', opacity: project.onChainRegistered ? 1 : .5 }}>
                    Contribute now
                  </button>
                  <button onClick={() => setShowAgentPrompt(true)} disabled={!project.onChainRegistered} style={{ width: '100%', marginTop: 8, background: 'none', border: 'none', fontSize: 12, color: 'var(--c-muted2)', cursor: project.onChainRegistered ? 'pointer' : 'default' }}>
                    or ask your Miron Agent to buy in →
                  </button>
                </>
              )}
              {isLive && showAgentPrompt && (
                <div style={{ marginTop: 16, padding: 14, borderRadius: 14, background: 'rgba(var(--c-fg-rgb),.05)', border: '1px solid rgba(var(--c-fg-rgb),.1)' }}>
                  <p style={{ fontSize: 12.5, color: 'var(--c-muted)', lineHeight: 1.5, marginBottom: 10 }}>
                    First come, first served, enforced on-chain. Ask your Miron Agent to buy in:
                  </p>
              ...
              {project.status === 'ended' && project.softcapMet && project.myContribution > 0 && claimStep !== 'done' && (
                <div style={{ marginTop: 12, padding: 14, borderRadius: 14, background: 'rgba(45,212,191,.08)', border: '1px solid rgba(45,212,191,.25)' }}>
                  <p style={{ fontSize: 12.5, color: 'var(--c-text)', lineHeight: 1.5, marginBottom: 10 }}>
                    Sale reached its goal — you can claim <strong>~{(project.myContribution / project.price).toLocaleString()} {project.sym}</strong> for your ${project.myContribution} contribution.
                  </p>
                  {project.tokensDeposited <= 0 && (
                    <p style={{ fontSize: 11.5, color: 'var(--c-muted2)', marginBottom: 8 }}>⏳ Project hasn&apos;t deposited tokens yet — check back later.</p>
                  )}
                  {claimStep === 'error' && <p style={{ fontSize: 11.5, color: '#fb6f84', marginBottom: 8 }}>{claimError}</p>}
                  <button
                    onClick={() => setClaimStep('pin')}
                    disabled={claimStep === 'progress' || project.tokensDeposited <= 0}
                    style={{ width: '100%', height: 38, borderRadius: 10, border: 'none', background: '#2dd4bf', color: '#04231f', fontSize: 12.5, fontWeight: 700, cursor: 'pointer', opacity: (claimStep === 'progress' || project.tokensDeposited <= 0) ? 0.5 : 1 }}
                  >
                    {claimStep === 'progress' ? 'Claiming…' : 'Claim tokens'}
                  </button>
                </div>
              )}
              {claimStep === 'done' && (
                <div style={{ marginTop: 12, padding: 14, borderRadius: 14, background: 'rgba(45,212,191,.08)', border: '1px solid rgba(45,212,191,.25)', textAlign: 'center' }}>
                  <p style={{ fontSize: 12.5, color: 'var(--c-text)', marginBottom: claimTxHash ? 6 : 0 }}>✅ Tokens claimed</p>
                  {claimTxHash && (
                    <a href={`https://testnet.arcscan.app/tx/${claimTxHash}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11.5, color: 'var(--c-indigo-light)', textDecoration: 'none' }}>
                      View on explorer ↗
                    </a>
                  )}
                </div>
              )}
              {project.status === 'ended' && !project.softcapMet && project.myContribution > 0 && refundStep !== 'done' && (
                <div style={{ marginTop: 12, padding: 14, borderRadius: 14, background: 'rgba(251,111,132,.08)', border: '1px solid rgba(251,111,132,.25)' }}>
                  <p style={{ fontSize: 12.5, color: 'var(--c-text)', lineHeight: 1.5, marginBottom: 10 }}>
                    This sale didn&apos;t reach its minimum goal — your contribution of <strong>${project.myContribution}</strong> is refundable.
                  </p>
                  {refundStep === 'error' && <p style={{ fontSize: 11.5, color: '#fb6f84', marginBottom: 8 }}>{refundError}</p>}
                  <button
                    onClick={() => setRefundStep('pin')}
                    disabled={refundStep === 'progress'}
                    style={{ width: '100%', height: 38, borderRadius: 10, border: 'none', background: '#fb6f84', color: '#fff', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', opacity: refundStep === 'progress' ? 0.6 : 1 }}
                  >
                    {refundStep === 'progress' ? 'Refunding…' : 'Claim refund'}
                  </button>
                </div>
              )}
              {refundStep === 'done' && (
                <div style={{ marginTop: 12, padding: 14, borderRadius: 14, background: 'rgba(45,212,191,.08)', border: '1px solid rgba(45,212,191,.25)', textAlign: 'center' }}>
                  <p style={{ fontSize: 12.5, color: 'var(--c-text)', marginBottom: refundTxHash ? 6 : 0 }}>✅ Refund confirmed</p>
                  {refundTxHash && (
                    <a href={`https://testnet.arcscan.app/tx/${refundTxHash}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11.5, color: 'var(--c-indigo-light)', textDecoration: 'none' }}>
                      View on explorer ↗
                    </a>
                  )}
                </div>
              ...
              {project.isOwnerOrAdmin && project.tokenAddress && (
                <div style={{ marginTop: 14, padding: 14, borderRadius: 14, background: 'rgba(var(--c-fg-rgb),.04)', border: '1px dashed rgba(var(--c-fg-rgb),.18)' }}>
                  <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.05em', textTransform: 'uppercase', color: 'var(--c-muted2)', marginBottom: 8 }}>
                    Deposit tokens to activate claims
                  </p>
                  <p style={{ fontSize: 11.5, color: 'var(--c-muted)', lineHeight: 1.6, marginBottom: 8 }}>
                    Call <code>depositTokens(bytes32,uint256)</code> on the contract below from your treasury wallet (the wallet you registered as this sale&apos;s treasury), after approving it to spend your token.
                  </p>
                  <div style={{ fontFamily: 'monospace', fontSize: 11, color: 'var(--c-text)', background: 'rgba(var(--c-fg-rgb),.05)', borderRadius: 8, padding: '8px 10px', marginBottom: 8, wordBreak: 'break-all' as const }}>
                    Contract: {project.contractAddress}<br />
                    saleId: {project.saleIdHash}<br />
                    Token: {project.tokenAddress}
                  </div>
                  <p style={{ fontSize: 12, color: 'var(--c-text)' }}>
                    Deposited so far: <strong>{project.tokensDeposited.toLocaleString()}</strong> {project.sym}
                    {' '}(hardcap needs ≈ {(project.target / project.price).toLocaleString()} {project.sym})
                  </p>
                </div>
              )}
              {project.audit && (
      ...
      {refundStep === 'pin' && (
        <AgentPinModal onSuccess={handleRefund} onCancel={() => setRefundStep('idle')} />
      )}

      {claimStep === 'pin' && (
        <AgentPinModal onSuccess={handleClaim} onCancel={() => setClaimStep('idle')} />
      )}

      {showContributeModal && (
        <LaunchpadContributeModal
          projectId={project.id}
          projectName={project.name}
          minContribution={project.minContribution}
          onClose={() => setShowContributeModal(false)}
          onSuccess={() => { setShowContributeModal(false); router.refresh() }}
        />
      )}
    </div>
  )
