...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!submission.token_address) {
      return NextResponse.json({ error: 'Submission has no token contract address — cannot approve' }, { status: 400 })
    }
    ...
    const { txHash, saleId } = await createSaleOnChain({
      ...
      minRaiseUsdc: submission.min_raise,
