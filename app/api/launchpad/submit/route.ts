...
export async function POST(request: NextRequest) {
  try {
    ...
    const { pin, name, sym, mark, accent, category, tagline, blurb, highlights, team, backers, socialLinks,
      tokenomics, vesting, price, target, cap, minContribution, supply, audit, startAt, endAt } = body

    if (!pin || typeof pin !== 'string' || !/^\d{6}$/.test(pin)) {
      return NextResponse.json({ error: 'PIN required' }, { status: 400 })
    }
    if (!name || !sym || !mark || !accent || !category || !tagline || !blurb || !supply || !startAt || !endAt) {
    ...
    if ([priceNum, targetNum, capNum, minNum].some(n => isNaN(n) || n <= 0)) {
      return NextResponse.json({ error: 'Price/target/cap/min must be positive numbers' }, { status: 400 })
    }
    ...
    const { data: inserted, error: insertErr } = await supabase.from('launchpad_submissions').insert({
      ...
      price: priceNum, target: targetNum, cap: capNum, min_contribution: minNum,
