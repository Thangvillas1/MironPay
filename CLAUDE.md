...
- `@circle-fin/bridge-kit` + `@circle-fin/provider-cctp-v2` + `@circle-fin/adapter-viem-v2` — installed with `--legacy-peer-deps`. Cross-chain USDC bridge (CCTPv2) between Arc Testnet and external testnets (Ethereum Sepolia, Base Sepolia). See `app/lib/circle-bridge-kit.ts`, `app/api/wallet/bridge/**`, `app/components/BridgeModal.tsx`. Withdraw (Arc → external) runs fully backend via `BridgeKit.bridge()`, reusing the existing `circleSwapAdapter` for the Arc leg plus a new backend-held relayer EOA (`BRIDGE_RELAYER_PRIVATE_KEY`) that submits the (permissionless) mint on the destination chain. Deposit (external → Arc) requires the user to connect a browser wallet (MetaMask) to sign the burn on their own chain — backend only prepares the unsigned burn calldata (`CCTPV2BridgingProvider.burn()`) and later completes attestation + mint after the browser submits the burn tx. Requires env var `BRIDGE_RELAYER_PRIVATE_KEY` (an EVM private key funded with testnet gas via faucet on each supported external chain).
