...
assert.match(dashboardSource, /Miron Agent is confirming the swap/)
assert.doesNotMatch(dashboardSource, /pointer-events-none fixed left-1\/2 top-5/)
assert.doesNotMatch(dashboardSource, /Check my balance|Send USDC to a friend|Suggestion chips/)
assert.match(dashboardSource, /confirmedAgentTransactions = \(agentWallet\?\.transactions \?\? \[\]\)/)
assert.doesNotMatch(dashboardSource, /const futureCosts = messages/)
assert.match(dashboardSource, /Autonomous session[\s\S]*agentSessionTimeLabel[\s\S]*Daily spend[\s\S]*X402 reachable/)
assert.doesNotMatch(dashboardSource, /\{sessionActive \? 'Online' : 'Offline'\}/)
const agentPageSource = fs.readFileSync('app/(app)/agent/page.tsx', 'utf8')
assert.doesNotMatch(agentPageSource, /Current balance\?|Should I swap USDC to EURC\?|My transaction history/)
