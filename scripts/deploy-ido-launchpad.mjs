 ...
 *
 * NOTE: this is a v2 deploy (adds softcap/minRaise + refund()) — any sale
 * approved under the previous IDO_LAUNCHPAD_CONTRACT address becomes
 * unreachable once this address replaces it, since launchpad-chain.ts reads
 * one global contract address, not per-sale. Fine pre-launch; flag to any
 * user relying on an old test sale.
 */
...
async function main() {
  ...
  const input = {
    ...
    // viaIR — Sale struct has enough fields that the auto-generated public
    // getter for the `sales` mapping hits "stack too deep" without it.
    settings: {
      viaIR: true,
  ...
  console.log('\n✅ IDOLaunchpad (v2, softcap+refund) deployed!')
